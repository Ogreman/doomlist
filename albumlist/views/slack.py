...
def handle_message_action(payload):
    try:
        flask.current_app.logger.debug('[access]: handling message action', callback_id=payload['callback_id'])
...
def handle_interactive_message(payload):
    try:
        ...
        flask.current_app.logger.debug('[access]: handling interactive message', action_name=action['name'])
                ...
                'attachments': [
                    {
                        ...
                        'text': review_text,
                        'callback_id': f'review_results_{album_id}',
                        'actions': [
                            {
                                'name': 'delete_review',
                                'text': 'Delete',
                                'type': 'button',
                                'value': f'{album_id}_{array_element}',
                            }
                        ] if payload['user']['id'] == review_user else [],
                    }
                    for array_element, (review_user, review_text) 
                    in enumerate([review.popitem() for review in album.reviews])
                ]
            }
            return flask.jsonify(response)
        elif 'delete_review' in action['name']:
            album_id, array_element = action['value'].split('_')
